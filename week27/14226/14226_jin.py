...
def bfs():
    ...
    q.append((1, 0))
    counter = list([0] * (S+2) for _ in range(S+2))  # 행: 이모지 수, 열: 클립보드 수, 값: cnt
    while q:
        emoji, cb = q.popleft()
        # 복사
        if 0 < emoji <= S+1 and counter[emoji][emoji] == 0:
        ...
        if cb:
            ...
            if 0 < x <= S+1 and counter[x][cb] == 0:
                q.append((x, cb))
                counter[x][cb] = counter[emoji][cb] + 1

        # 한 개 삭제
        if 0 < emoji-1 and counter[emoji-1][cb] == 0:
            q.append((emoji-1, cb))
